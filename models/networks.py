...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02,
    ...
    if netG == 'Recurrent_unet_32':
        net = RecurrentUnetGenerator(input_nc, output_nc, 5, ngf, norm_layer=norm_layer, use_dropout=use_dropout)
    elif netG == 'Recurrent_unet_64':
        net = RecurrentUnetGenerator(input_nc, output_nc, 6, ngf, norm_layer=norm_layer, use_dropout=use_dropout)
    elif netG == 'Recurrent_unet_128':
        net = RecurrentUnetGenerator(input_nc, output_nc, 7, ngf, norm_layer=norm_layer, use_dropout=use_dropout)
    elif netG == 'Recurrent_unet_256':
        net = RecurrentUnetGenerator(input_nc, output_nc, 8, ngf, norm_layer=norm_layer, use_dropout=use_dropout)
    elif netG == 'Recurrent_resnet_3blocks':
        net = RecurrentResnetGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout,
                                       n_blocks=3)
    elif netG == 'Recurrent_resnet_3blocks':
        net = RecurrentResnetGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout,
                                       n_blocks=6)
    elif netG == 'Recurrent_resnet_9blocks':
        net = RecurrentResnetGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout,
                                       n_blocks=9)
...
def define_D(input_nc, ndf, netD, n_layers_D=3, norm='batch', init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    Parameters:
        input_nc (int)     -- the number of channels in input imagesfbasic
...
###############################################################################
# Recurrent Unet Generator
###############################################################################
class RecurrentUnetGenerator(nn.Module):

    def __init__(self, input_nc, output_nc, num_downs, ngf=64, norm_layer=nn.BatchNorm3d, use_dropout=False):
        super(RecurrentUnetGenerator, self).__init__()
        self.S_Encoder = UnetGenerator(input_nc * 2, input_nc, num_downs, ngf, norm_layer=norm_layer,
                                       use_dropout=use_dropout)
        self.S_Decoder = UnetGenerator(input_nc, output_nc, num_downs, ngf, norm_layer=norm_layer,
                                       use_dropout=use_dropout)

    def forward(self, input, h):
        x = torch.cat([h, input], 1)
        h_next = self.S_Encoder(x)
        y = self.S_Decoder(h_next)
        return y, h_next
...
###############################################################################
# Recurrent Resnet Generator
###############################################################################
class RecurrentResnetGenerator(nn.Module):

    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm3d, use_dropout=False, n_blocks=6,
                 padding_type='replicate'):
        super(RecurrentResnetGenerator, self).__init__()
        self.S_Encoder = ResnetGenerator(input_nc * 2, input_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout,
                                         n_blocks=n_blocks, padding_type=padding_type)

        self.S_Decoder = ResnetGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout,
                                         n_blocks=n_blocks, padding_type=padding_type)
...
class smooth_l1_loss(nn.Module):
    ...
    def __call__(self, input, target, beta=0.06, size_average=True):
        """
